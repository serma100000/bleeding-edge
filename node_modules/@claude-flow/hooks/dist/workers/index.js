 ...
 */
export function createV3ProgressWorker(projectRoot) {
    return async () => {
        const startTime = Date.now();
        const v3Path = path.join(projectRoot, 'v3');
        ...
        const overallProgress = Math.round((cliProgress * 0.25) +
            (mcpProgress * 0.25) +
            (hooksProgress * 0.20) +
            (pkgProgress * 0.15) +
            (dddProgress * 0.15));
 ...
 */
async function countFilesRecursive(dir, ext) {
    let count = 0;
    try {
        const entries = await fs.readdir(dir, { withFileTypes: true });
        for (const entry of entries) {
